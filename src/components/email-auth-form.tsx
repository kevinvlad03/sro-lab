...
export function EmailAuthForm({
  ...
  const [state, formAction, pending] = useActionState<AuthState, FormData>(
    authAction,
    params.error ? { error: decodeURIComponent(params.error) } : null,
  );
  ...
  return (
    ...
    >
      <div className="relative mb-4 grid grid-cols-2 gap-1 rounded-full border border-border bg-surface p-1">
        {tabs.map((t) => (
          <button
            key={t.id}
            type="button"
